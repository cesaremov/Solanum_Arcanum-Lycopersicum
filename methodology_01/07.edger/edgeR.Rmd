...
```{r}
# Get counts table
countsTab <- read.table("../06.GetCounts/GetCounts/counts.tab", sep = "\t", header = TRUE, row.names = 1, quote = "")
dim(countsTab)
...
```{r}
# Get trx ref_gene map
trx_ref_geneTab <- read.table("../00.AnnotateGenome/00.GetSeqs/GetSeqs/trx_ref_gene.tab", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
trx_ref_geneTab <- trx_ref_geneTab[, c("trx_id", "ref_gene.value")]
trx_ref_geneTab$ref_gene <- trx_ref_geneTab$ref_gene.value
trx_ref_geneTab <- data.frame(apply(trx_ref_geneTab, 2, function(x) gsub("mRNA\\:|gene\\:", "", x)), stringsAsFactors = FALSE)
row.names(trx_ref_geneTab) <- trx_ref_geneTab$ref_gene.value
```
...
ids <- intersect(row.names(countsTab), row.names(trx_ref_geneTab))
dge <- DGEList(counts = countsTab[ids,], group = Group, genes = trx_ref_geneTab[ids,])
...
for (contrast in contrasts) {
  ...
  heatmap.2(log(topDECpmTab+1), trace = "n", 
            RowSideColors = ifelse(deTab[row.names(topDECpmTab), "logFC"] > 0, "firebrick3", "dodgerblue4"),
